...
df_train_actual = pd.DataFrame()
df_val_actual = pd.DataFrame() 
df_test_actual = pd.DataFrame()
...
for i in range(70117, 91291, 24):
    ...
    if test_end < 91291:
        ...
        df_train_actual = pd.concat([df_train_actual, pd.DataFrame({train_str: TrainingOutput})], ignore_index=False, axis=1)
        df_val_actual = pd.concat([df_val_actual, pd.DataFrame({val_str: ValidationOutput})], ignore_index=False, axis=1)
        df_test_actual = pd.concat([df_test_actual, pd.DataFrame({test_str: TestingOutput})], ignore_index=False, axis=1)
...
df_train_actual.to_csv("../TrainingShort/TrainingActual_Short.csv", index = False)
df_val_actual.to_csv("../ValidationShort/ValidationActual_Short.csv", index = False)
